...
\section{Data}
The preprocessed dataset used in this study the one obtained in the recordings in the study by Mizuseki et. al \cite{Mizuseki2012}. Data analysis was done by \todo{test comment} custom written MATLAB code. A MATLAB class library was developed as a part of the work. 
...
%\begin{equation}
%P(\mathbf{n} | \mathbf{x}) = \prod_{i = 1}^{N} P(n_{i} | \mathbf{x}) = \prod_{i = 1}^{N} \frac{(\tau f_{i}(\mathbf{x}))^{n_{i}}}{n_{i} !} e^{-\tau f_{i}		% (\mathbf{x})}P(\mathbf{n} | \mathbf{x}) = \prod_{i = 1}^{N} P(n_{i} | \mathbf{x}) = \prod_{i = 1}^{N} \frac{(\tau f_{i}(\mathbf{x}))^{n_{i}}}{n_{i} !} \emph{e}^{-\tau f_{i}(\mathbf{x})}
%\end{equation}
\[ 
